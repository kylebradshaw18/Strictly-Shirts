...
<div class="container">
    ...
    <div class="col-md-2" id="leftCol">
        <form action="categoryPage.php" method="get">
            <ul class="nav nav-stacked text-center" id="sidebar">
                ...
                <li>
                    <button type="submit" class="btn btn-sm btn-info" value="Search" title="Search For Shirts">Search</button>
                </li>
            </ul>
        </form>
    </div>
    ...
    <?php //run main query and loop through them
        ...
        $mainSql .= " WHERE 1 = 1 "; //Put this here so that we can just add and clauses to the query
                ...
                <div class="col-md-3 col-sm-12 productsSpace">
                    <div class="col-md1 simpleCart_shelfItem">
                        ...
                        <?php
                            ...
                            /*
                            $inventorySql .= " FROM `inventory` INNER JOIN `colors` ON `inventory`.`colorId` = `colors`.`colorId` ";
                            $inventorySql .= "                  INNER JOIN `sizes` ON `inventory`.`sizeId` = `sizes`.`sizeId` ";
                            $inventorySql .= "                  INNER JOIN `types` ON `inventory`.`typeId` = `types`.`typeId` ";*/
                        	...
                        	//array_push($productInformationArray, $inventoryInformation);
                        ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <?php if(isset($_SESSION['personId']) && !empty($_SESSION['personId'])) { ?>
                    <button href="#" class="btn btn-primary" onclick="addToCart()" id="productModalAddToCart">Add to Cart</button>
                <?php } else { ?>
